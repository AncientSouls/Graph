...
exports.Graph = exports.factoryObjectGraph = undefined;
...
function factoryObjectGraph(ParentClassGraph) {
   ...
   */
  var ObjectGraph = function (_ParentClassGraph) {
    _inherits(ObjectGraph, _ParentClassGraph);
     ...
     */
    function ObjectGraph(collection, fields, config) {
      _classCallCheck(this, ObjectGraph);

      var _this = _possibleConstructorReturn(this, (ObjectGraph.__proto__ || Object.getPrototypeOf(ObjectGraph)).apply(this, arguments));
    ...
    _createClass(ObjectGraph, [{
       ...
       * @return {number|string} id;
       */
      value: function _insertModifier(link) {
        var _modifier = {};
        for (var f in link) {
          if (this.fields[f]) {
      ...
      key: '_updateModifierPush',
      value: function _updateModifierPush(data, item) {
        data.push(item);
      }
      ...
      key: '_updateModifierAdd',
      value: function _updateModifierAdd(data, item) {
        if (_lodash2.default.isArray(item)) {
          for (var i in item) {
            this._updateModifierAdd(data, item[i]);
          }
        } else {
          var index = _lodash2.default.indexOf(data, item);
          if (index < 0) {
      ...
      key: '_updateModifierRemove',
      value: function _updateModifierRemove(data, item) {
        if (_lodash2.default.isArray(item)) {
          for (var i in item) {
            this._updateModifierRemove(data, item[i]);
          }
        } else {
          _lodash2.default.remove(data, function (value) {
            return value == item;
          });
        }
      }
      ...
      key: '_updateModifier',
      value: function _updateModifier(modifier, result) {
        for (var m in modifier) {
          if (this.fields[m]) {
            if (typeof modifier[m] == 'undefined') {
              delete result[this.fields[m]];
            } else {
              if (_typeof(modifier[m]) == 'object') {
                if (_lodash2.default.isArray(modifier[m])) {
                  result[this.fields[m]] = modifier[m];
                } else {
                  if (!_lodash2.default.isArray(result[this.fields[m]])) {
      ...
      value: function query(selector) {
        ...
        if (type == 'string' || type == 'number') {
          ...
          return function (doc) {
            if ((typeof doc === 'undefined' ? 'undefined' : _typeof(doc)) != 'object') return false;
            for (var m in selector) {
              if (_this3.fields[m]) {
                if (typeof selector[m] == 'undefined') {
                  if (doc.hasOwnProperty(_this3.fields[m])) return false;
                } else {
                  if (_lodash2.default.isArray(doc[_this3.fields[m]])) {
                    if (_lodash2.default.isArray(selector[m])) {
                      for (var s in selector[m]) {
                        if (!_lodash2.default.includes(doc[_this3.fields[m]], selector[m][s])) {
                          return false;
                        }
                      }
                    } else {
                      if (!_lodash2.default.includes(doc[_this3.fields[m]], selector[m])) {
                        return false;
                      }
                    }
                  } else {
          ...
          if (_options2.sort) {
            ...
            for (var s in _options2.sort) {
              if (this.fields[s]) {
                ...
                _options.sort.orders.push(_options2.sort[s] ? 'asc' : 'desc');
              }
            }
          }
      ...
      key: 'get',
      value: function get(selector, options, callback) {
        var results = this.fetch(selector, options, function (error, results) {
      ...
      value: function _fetch(selector, options) {
        ...
        if (_options.sort) documents = _lodash2.default.orderBy(documents, _options.sort.keys, _options.orders);
      ...
      key: 'count',
      value: function count(selector, options, callback) {
        var links = this.fetch(selector, options);
...
var ObjectGraph = factoryObjectGraph(_graph.Graph);

exports.factoryObjectGraph = factoryObjectGraph;
