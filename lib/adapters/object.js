'use strict';Object.defineProperty(exports,'__esModule',{value:true});exports.Graph=exports.factoryObjectGraph=undefined;var _typeof=typeof Symbol==='function'&&typeof Symbol.iterator==='symbol'?function(obj){return typeof obj}:function(obj){return obj&&typeof Symbol==='function'&&obj.constructor===Symbol?'symbol':typeof obj};var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if('value'in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor}}();var _graph=require('../graph.js');var _lodash=require('lodash');var _lodash2=_interopRequireDefault(_lodash);var _fbemitter=require('fbemitter');function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function')}}function _possibleConstructorReturn(self,call){if(!self){throw new ReferenceError('this hasn\'t been initialised - super() hasn\'t been called')}return call&&(typeof call==='object'||typeof call==='function')?call:self}function _inherits(subClass,superClass){if(typeof superClass!=='function'&&superClass!==null){throw new TypeError('Super expression must either be null or a function, not '+typeof superClass)}subClass.prototype=Object.create(superClass&&superClass.prototype,{constructor:{value:subClass,enumerable:false,writable:true,configurable:true}});if(superClass)Object.setPrototypeOf?Object.setPrototypeOf(subClass,superClass):subClass.__proto__=superClass}/**
 ...
 */function factoryObjectGraph(ParentClassGraph){/**
   ...
   */var ObjectGraph=function(_ParentClassGraph){_inherits(ObjectGraph,_ParentClassGraph);/**
     ...
     */function ObjectGraph(collection,fields){_classCallCheck(this,ObjectGraph);var _this=_possibleConstructorReturn(this,(ObjectGraph.__proto__||Object.getPrototypeOf(ObjectGraph)).call(this));_this.collection=collection;_this.fields=fields;_this.emitter=new _fbemitter.EventEmitter;return _this}/**
     ...
     * @param {Link} link
     * @return {number|string} id;
     */value:function _insertModifier(link){var _modifier={};for(var f in link){if(this.fields[f]){_modifier[this.fields[f]]=link[f]}}return _modifier}},{key:'insert',/**
     ...
     * @return {number|string} [id]
     */value:function insert(link,callback,context){this.callback;var _modifier=this._insertModifier(link);var index,error,id;try{index=this.collection.push(_modifier)-1;if(!this.collection[index].hasOwnProperty(this.fields['id'])){id=this.collection[index][this.fields['id']]=this._idGenerator(index,this.collection[index])}this.emitter.emit('insert',this.collection[index])}catch(_error){error=_error}if(callback){callback(error,id)}return id}/**
     ...
     */},{key:'on',value:function on(event,callback){var _this4=this;if(event=='insert'){this.emitter.addListener('insert',function(document){callback(undefined,_this4._generateLink(document))})}if(event=='update'){this.emitter.addListener('update',function(newDocument,oldDocument){callback(_this4._generateLink(oldDocument),_this4._generateLink(newDocument))})}if(event=='remove'){this.emitter.addListener('remove',function(document){callback(_this4._generateLink(document),undefined)})}if(event=='link'){this.emitter.addListener('insert',function(document){callback(undefined,_this4._generateLink(document))});this.emitter.addListener('update',function(newDocument,oldDocument){callback(_this4._generateLink(oldDocument),_this4._generateLink(newDocument))})}if(event=='unlink'){this.emitter.addListener('update',function(newDocument,oldDocument){callback(_this4._generateLink(oldDocument),_this4._generateLink(newDocument))});this.emitter.addListener('remove',function(document){callback(_this4._generateLink(document),undefined)})}}/**
